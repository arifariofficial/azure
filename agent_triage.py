...
if project_endpoint is None or model_deployment is None:
    ...
    if model_deployment is None:
        print("MODEL_DEPLOYMENT_NAME environment variable is not set.")
    exit(1)
...
with agents_client:
    ...
    priority_agents_instructions = """
    Assess how urgent a ticket is based on it's description.
    ...
    Only output the urgency level and  a very breif explaination.
    """

    priority_agent = agents_client.create_agent(
        model=model_deployment,
        name=priority_agent_name,
        instructions=priority_agents_instructions,
    )
    ...
    Base your answer on the content of the ticket. Respond with the team name and a very explanation
    """
    ...
    Base your estimate on the complexity implied by the ticket. Respond with the effort level and a breif justification. 
    """
    ...
    priority_agent_tool = ConnectedAgentTool(
        ...
        name=priority_agent_name,
        description="Detemines which team should take the ticket",
    )
    ...
    effort_agent_tool = ConnectedAgentTool(
        id=effort_agent.id,
        name=effort_agent_name,
        description="Detemines the effort required to complete the ticket",
    )

    # Create an agent to triage support ticket processing by using connected agents
    triage_agent_name = "triage-agent"
    triage_agent_instructions = """
    Triage the given ticket. Use the connected tools to detemine the ticket's priority,
    ...
    messsage = agents_client.messages.create(
